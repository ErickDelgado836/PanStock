...
interface CategoryProductTableProps {
  ...
  getLastMovementInfo: (productId: string) => {
    typeText: string;
    qtyText: string;
    detail: string;
    date: string;
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {/* Desktop Scroll Controls when table overflows */}
          {hasOverflow && (
            <div className="hidden sm:flex items-center gap-1 bg-white border border-slate-200 p-1 rounded-xl shadow-2xs mr-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase px-1.5 select-none">
                Mover tabla:
              </span>
              <button
                type="button"
                ...
                className={`p-1.5 rounded-lg border transition-all ${
                  canScrollLeft
                    ? 'bg-slate-50 hover:bg-red-50 text-slate-700 hover:text-red-600 border-slate-200 hover:border-red-300 cursor-pointer active:scale-95'
                ...
                {catProducts.map((prod) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 font-bold max-w-[280px] whitespace-normal">
                        <div className="leading-snug">
                          <span className={hasStock ? 'text-slate-900' : 'text-slate-500 font-medium'}>
                            {prod.name}
                          </span>
                          {!hasStock && (
                            <span className="ml-2 inline-block px-1.5 py-0.5 text-[10px] bg-slate-200/60 text-slate-500 rounded font-normal whitespace-nowrap">
                              Sin stock local
                            </span>
                          )}
                        </div>
            ...
            return displayCategories.map((cat) => {
              ...
              return (
                <CategoryProductTable
                  ...
                  warehouseAuditMap={warehouseAuditMap}
                  getLastMovementInfo={getLastMovementInfo}
                  onNavigateToAuditReport={onNavigateToAuditReport}
