...
export const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  return (
    ...
    >
        ...
        className={`w-full min-h-[42px] px-3 py-2 bg-white border rounded-xl text-left transition-all flex items-center justify-between gap-2 shadow-2xs ${
          disabled
            ...
            : `cursor-pointer hover:bg-slate-50 ${
                isOpen ? `ring-2 ${borderFocusClasses}` : 'border-slate-300 hover:border-slate-400'
              }`
        }`}
      >
          ...
          {selectedOption ? (
            <div className="flex items-center gap-2 overflow-hidden flex-1 min-w-0">
              {selectedOption.badge && (
                <span
                  className={`px-2 py-0.5 border rounded-md font-black text-[11px] shrink-0 ${badgeBgClasses}`}
                >
                  {selectedOption.badge}
                </span>
              )}
      ...
      {/* Desktop Floating Dropdown Panel (sm: and above) */}
      {isOpen && !disabled && (
        <div className="hidden sm:flex absolute left-0 right-0 top-full mt-1.5 bg-white/95 backdrop-blur-xl border border-slate-300 rounded-2xl shadow-2xl z-50 p-2 space-y-1.5 max-h-[340px] flex-col animate-in fade-in zoom-in-95 duration-150 min-w-full">
          ...
          {/* Options List */}
          <div className="overflow-y-auto space-y-1 flex-1 pr-0.5 custom-scrollbar touch-pan-y max-h-[260px]">
      ...
      {/* Mobile Bottom Sheet Modal (Portaled into document.body so it NEVER clips or goes behind menus) */}
      {isOpen &&
        !disabled &&
        typeof document !== 'undefined' &&
        createPortal(
          <div className="sm:hidden fixed inset-0 z-[99999] flex flex-col justify-end bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-150 p-2.5 pb-4">
            ...
            <div className="relative z-10 w-full max-h-[82vh] bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-6 duration-200">
              ...
              {/* Quick Search inside Mobile Sheet (NO autoFocus to prevent mobile keyboard from intrusively opening) */}
              {showSearch && (
                <div className="p-3 border-b border-slate-100 bg-white shrink-0">
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400/20"
                    />
                  </div>
                </div>
              )}
              ...
              {/* Mobile Options List */}
              <div className="p-2 space-y-1.5 overflow-y-auto flex-1 overscroll-contain max-h-[58vh]">
                {filteredOptions.length === 0 ? (
                  ...
                  filteredOptions.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-2xl cursor-pointer transition-all flex items-center justify-between gap-3 border min-h-[48px] ${
                          opt.disabled
                            ? 'opacity-50 cursor-not-allowed bg-slate-50 border-transparent text-slate-400'
                            : isSelected
                            ? `${selectedBgClasses} font-bold shadow-xs`
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                          {opt.badge && (
                            <span
                              className={`px-2 py-0.5 border rounded-md font-black text-xs shrink-0 ${
                                isSelected
                                  ? badgeBgClasses
                                  : 'bg-slate-100 text-slate-800 border-slate-200'
                              }`}
                        ...
                        {isSelected && <Check className="w-5 h-5 shrink-0 text-slate-800" />}
                      </button>
                    );
                  })
                )}
